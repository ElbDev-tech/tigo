  ...
  const loadData = async () => {
    try {
      const [instalacionesRes, clientesRes] = await Promise.all([
        supabase.from('instalaciones').select('*').order('fecha_instalacion', { ascending: false }),
      ...
      const instalacionesConClientes = instalacionesRes.data.map((inst) => ({
        ...inst,
        cliente: clientesRes.data.find((c) => c.id === inst.id_cliente),
      }));
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (editingInstalacion) {
        ...
        const { error } = await supabase.from('instalaciones').insert(formData);
        if (error) throw error;
      }
      setShowModal(false);
      resetForm();
